...
2. 调用 `list_projects` 查看已有项目
3. 若新建：调用 `create_project`，填入项目名称、题材、目标受众
4. 若选择已有：调用 `get_project` 获取详情
5. 必要时调用 `update_project` 调整选题参数
...
3. 按创作方案内容结构生成完整 JSON（见第 4 节）
4. 调用 `save_creative_plan` 保存创作方案
...
3. 为主角、重要配角、反派体系调用 `create_character` 或 `update_character`
4. 调用 `set_character_relations` 建立角色关系图谱
...
2. 调用 `get_episode_script`、`list_characters`、`list_scenes` 获取上下文
3. 按单集微结构撰写剧本（开篇钩子 → 中段推进 → 结尾悬念）
4. 调用 `save_episode_script` 保存剧本

**Output**：第 N 集剧本摘要，提示用户继续「/分集 N+1」或修改。
...
| storyboard-guide.md | 分镜指南 | 分镜创作时 |
| visual-style-guide.md | 视觉风格 | 分镜创作时 |
